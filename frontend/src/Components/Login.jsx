...
export default function FormDialog() {
  ...
  const handleContinueRegistration = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/auth/otplogin",
        {
          method: "POST",
          headers: {
  ...
  const handleOtp = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/auth/otpverify?mobile_number=${mobile}`,
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
  ...
  const getProfile = async () => {
    try {
      ...
      const json = await response.json();
      if (json.success) {
        dispatch(getUserName(json.data.user.name));
        setOpen(false);
      }
    } catch (error) {
      console.log(error);
        ...
        <DialogContent>
          {!loginModalOtp && !loginModalEmailOtp && (
            <div className={styles.personalBusiness}>
              ...
              <div
                onClick={handleToggleModal}
                className={loginModal ? styles.blackColor : styles.linearBackground}
              >
                Login
              </div>
            </div>
            ...
            <>
              ...
              >
                ...
                <div className={styles.terms_text}>
                  By proceeding, you agree to SKY NORA TRAVELS's
